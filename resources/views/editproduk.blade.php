...
@section('title', 'Edit Produk - GreSOY')

@section('content')
<div class="w-100 p-3 p-md-4">

    <div class="mb-4">
        <span class="fs-5 fw-semibold">Admin &gt; Produk &gt; Edit Produk</span>
    </div>
    ...
    <div class="row justify-content-center">
        <div class="col-12 col-md-8 col-lg-6">
            ...
            <form action="{{ route('produk.update', $product->id) }}" method="POST" autocomplete="off">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <input type="number" name="jumlah_susu"
                               class="form-control input-rounded"
                               value="{{ old('jumlah_susu', $product->jumlah_susu) }}"
                               min="0" required>
                ...
                <div class="mb-3">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <input type="number" name="jumlah_gula"
                               class="form-control input-rounded"
                               value="{{ old('jumlah_gula', $product->jumlah_gula) }}"
                               min="0" required>
                ...
                <div class="mb-3">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <input type="number" name="jumlah_gula_tropicana"
                               class="form-control input-rounded"
                               value="{{ old('jumlah_gula_tropicana', $product->jumlah_gula_tropicana) }}"
                               min="0" required>
                        <span class="unit-text">Gram</span>
                    </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="jenis_powder" class="form-select input-rounded" required>
                        ...
                        @foreach($powderOptions as $powderName)
                            <option value="{{ $powderName }}"
                                {{ old('jenis_powder', $product->jenis_powder) == $powderName ? 'selected' : '' }}>
                                {{ $powderName }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <input type="number" name="jumlah_powder"
                               class="form-control input-rounded"
                               value="{{ old('jumlah_powder', $product->jumlah_powder) }}"
                               min="0" required>
                        <span class="unit-text">Gram</span>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    const redirect = () => window.location.href = "{{ route('produk.index') }}";
